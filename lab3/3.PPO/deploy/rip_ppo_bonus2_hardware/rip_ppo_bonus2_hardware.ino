/*
 * Rotary inverted pendulum direct PPO Bonus-2 controller.
 *
 * One uploaded 7 -> 64 -> 64 -> 1 Actor controls the full downward swing-up
 * and upright balance trajectory.  Hidden layers use ReLU and the final action
 * uses Tanh, exactly matching the training package.
 *
 * Observation:
 *   [sin(theta), cos(theta), theta_dot,
 *    sin(alpha), cos(alpha), alpha_dot, previous_controller_pwm/model_pwm_scale]
 *
 * Large-angle velocity uses wrapped finite differences.  Near upright the
 * velocity components blend smoothly into the fixed 5 ms Luenberger observer.
 * Control and sensing run at 200 Hz; telemetry is 20 Hz; idle monitor is 4 Hz.
 *
 * Serial protocol version: 22
 */
#include <Arduino.h>
...
static const uint32_t IDLE_TELEMETRY_DIVIDER = 50;
static const uint32_t RUN_TELEMETRY_DIVIDER = 10;
...
static const int MODE_PPO_DIFF = 1;
static const int MODE_PPO_BLEND = 2;
static const int MODE_PPO_OBSERVER = 3;
...
struct PPOBonus2FloatModel {
  ...
  float theta_dot_clip;
  float alpha_dot_clip;
};

static const uint32_t MODEL_BLOB_BYTES =
    (MODEL_H1 * MODEL_INPUT + MODEL_H1 +
     MODEL_H2 * MODEL_H1 + MODEL_H2 +
     MODEL_OUTPUT * MODEL_H2 + MODEL_OUTPUT + 3) * sizeof(float);
static_assert(sizeof(PPOBonus2FloatModel) == MODEL_BLOB_BYTES,
              "PPOBonus2FloatModel layout must match Python float_blob");

static ControllerConfig g_cfg = {
  0.0f, 150.0f, 20.0f, 35.0f, 0.18f, 0.25f,
  3110, 990, 0.00583730846f, 1
};
static PPOBonus2FloatModel g_model;
...
static uint32_t g_run_telemetry_divider = 0;
...
static bool validateConfig(const ControllerConfig& c) {
  if (!(c.duration == 0.0f || (c.duration >= 0.1f && c.duration <= 600.0f))) return false;
...
static void buildCompactObservation(const float state[4], float out[MODEL_INPUT]) {
  out[0] = sinf(state[0]);
  out[1] = cosf(state[0]);
  out[2] = clampf(state[1], -g_model.theta_dot_clip, g_model.theta_dot_clip);
  out[3] = sinf(state[2]);
  out[4] = cosf(state[2]);
  out[5] = clampf(state[3], -g_model.alpha_dot_clip, g_model.alpha_dot_clip);
  out[6] = clampf(g_previous_controller_pwm / g_model.model_pwm_scale, -1.0f, 1.0f);
}

static bool ppoBonus2Forward(const float rawObs[MODEL_INPUT], float* actionNorm, float* policyRaw) {
  if (!g_model_loaded) return false;
  for (int i = 0; i < MODEL_H1; ++i) {
    float acc = g_model.b0[i];
    for (int j = 0; j < MODEL_INPUT; ++j) acc += g_model.w0[i][j] * rawObs[j];
    g_h1[i] = acc > 0.0f ? acc : 0.0f;
  }
  for (int i = 0; i < MODEL_H2; ++i) {
    float acc = g_model.b1[i];
    for (int j = 0; j < MODEL_H1; ++j) acc += g_model.w1[i][j] * g_h1[j];
    g_h2[i] = acc > 0.0f ? acc : 0.0f;
  }
...
static bool modelValuesValid() {
  ...
  if (!(isfinite(g_model.theta_dot_clip) && g_model.theta_dot_clip >= 0.1f && g_model.theta_dot_clip <= 500.0f)) return false;
  if (!(isfinite(g_model.alpha_dot_clip) && g_model.alpha_dot_clip >= 0.1f && g_model.alpha_dot_clip <= 500.0f)) return false;
  for (int i = 0; i < MODEL_H1; ++i) {
...
static void startControl(uint32_t runId) {
  if (g_binary_session_active || g_model_loading) { Serial.println("ERR,RUN_UPLOAD_ACTIVE"); return; }
  if (!g_config_valid) { Serial.println("ERR,RUN_CONFIG_REQUIRED"); return; }
  if (!g_model_loaded) { Serial.println("ERR,RUN_MODEL_REQUIRED"); return; }
  if (g_armed) {
    if (runId == g_run_id) {
      Serial.print("ARMED,"); Serial.print(g_run_id); Serial.print(",");
      Serial.print(g_pwm); Serial.print(","); Serial.print(g_last_action_norm, 7);
      Serial.print(","); Serial.println(g_last_policy_raw, 7);
    } else Serial.println("ERR,RUN_ALREADY_RUNNING");
    return;
  }

  // Capture one coherent initial measurement and make the first PPO Bonus-2 decision
  // synchronously inside GO.  The old v21 path only set g_armed and waited for
  // a later loop tick; a runtime rejection before that tick looked like a
  // successful GO with a motionless motor.
  int pot;
  int32_t enc;
  ControllerConfig c;
  noInterrupts();
  ...
  const float alpha0 = alphaFromPot(pot, c);
  float initialState[4] = {theta0, 0.0f, alpha0, 0.0f};
  float initialObs[MODEL_INPUT];

  g_previous_controller_pwm = 0.0f;
  buildCompactObservation(initialState, initialObs);
  float initialAction = 0.0f;
  float initialRaw = 0.0f;
  if (!ppoBonus2Forward(initialObs, &initialAction, &initialRaw)) {
    motorBrake();
    Serial.println("ERR,RUN_MODEL_INFERENCE");
    return;
  }

  float initialCommand = initialAction * g_model.model_pwm_scale;
  initialCommand = clampf(initialCommand, -c.safety_pwm_limit, c.safety_pwm_limit);
  int controllerPwm = (int)lroundf(initialCommand);
  int initialPwmApplied = c.motor_sign * controllerPwm;
  initialPwmApplied = constrain(initialPwmApplied,
                                -(int)c.safety_pwm_limit,
                                (int)c.safety_pwm_limit);

  noInterrupts();
  g_step = 1;
  g_run_telemetry_divider = 0;
  g_run_id = runId;
  g_pwm = initialPwmApplied;
  g_mode = MODE_PPO_DIFF;
  g_blend = 0.0f;
  g_armed = true;
  interrupts();
  ...
  g_prev_theta = theta0;
  g_prev_alpha = alpha0;
  g_theta_unwrapped = theta0;
  g_theta_dot_lpf = 0.0f;
  g_alpha_dot_lpf = 0.0f;
  g_velocity_ready = true;
  g_observer_active = false;
  g_previous_controller_pwm = initialCommand;
  g_last_action_norm = initialAction;
  g_last_policy_raw = initialRaw;
  observerReset();

  // Apply the actual first policy command before acknowledging GO.
  motorDrive(initialPwmApplied);

  Serial.print("ARMED,"); Serial.print(runId); Serial.print(",");
  Serial.print(initialPwmApplied); Serial.print(",");
  Serial.print(initialAction, 7); Serial.print(",");
  Serial.println(initialRaw, 7);
}
static void printStatus() {
  ...
  Serial.print(",");
  Serial.print(g_run_id);
  Serial.print(",");
  Serial.println(g_step);
}
...
static void applyConfig(const String& line) {
  ...
  if (n != 13 || f[1].toInt() != PROTOCOL_VERSION) {
  ...
  c.observer_enter_deg = f[5].toFloat();
  c.observer_exit_deg = f[6].toFloat();
  c.observer_blend_alpha = f[7].toFloat();
  c.velocity_lpf = f[8].toFloat();
  c.pot_up = f[9].toInt();
  c.pot_down = f[10].toInt();
  c.theta_rad_per_count = f[11].toFloat();
  c.motor_sign = f[12].toInt();
  if (!validateConfig(c)) {
...
static void handleCommand(const String& raw) {
  ...
  if (line == "HELLO") { Serial.println("READY,RIP_PPO_BONUS2_64_DIRECT_HW_GO_FIX,22"); return; }
  if (line.startsWith("PING,")) { Serial.print("PONG,"); Serial.print(line.substring(5)); Serial.print(","); Serial.print(g_armed ? 1 : 0); Serial.print(","); Serial.print(g_run_id); Serial.print(","); Serial.println(g_step); return; }
...
static void processControlTick() {
  ...
  if (c.duration > 0.0f && runTime >= c.duration) {
    stopControl();
    Serial.print("DONE,");
  ...
  if (!isfinite(g_theta_unwrapped) || !isfinite(alpha) ||
      ...
      fabsf(g_theta_dot_lpf) > 2.0f * g_model.theta_dot_clip ||
      fabsf(g_alpha_dot_lpf) > 2.0f * g_model.alpha_dot_clip) {
    stopControl();
  ...
  float enter = c.observer_enter_deg * PI_F / 180.0f;
  float exitAngle = c.observer_exit_deg * PI_F / 180.0f;
  ...
  // Keep the observer updated, but use it only through a near-upright blend.
  observerUpdate(g_previous_controller_pwm, g_theta_unwrapped, alpha);
  float target = g_observer_active ? 1.0f : 0.0f;
  float blend = g_blend + c.observer_blend_alpha * (target - g_blend);
  blend = clampf(blend, 0.0f, 1.0f);

  float controlState[4];
  controlState[0] = g_theta_unwrapped;
  controlState[1] = (1.0f - blend) * g_theta_dot_lpf + blend * g_xhat[1];
  controlState[2] = wrapToPi(alpha);
  controlState[3] = (1.0f - blend) * g_alpha_dot_lpf + blend * g_xhat[3];

  float compactObs[MODEL_INPUT];
  buildCompactObservation(controlState, compactObs);
  float actionNorm = 0.0f;
  float policyRaw = 0.0f;
  if (!ppoBonus2Forward(compactObs, &actionNorm, &policyRaw)) {
    stopControl();
    Serial.println("ERR,MODEL_INFERENCE");
    return;
  }
  float command = actionNorm * g_model.model_pwm_scale;
  ...
  int mode = blend <= 0.01f ? MODE_PPO_DIFF :
             (blend >= 0.99f ? MODE_PPO_OBSERVER : MODE_PPO_BLEND);
  noInterrupts();
  g_step = step + 1;
  g_pwm = pwmApplied;
  g_mode = mode;
  ...
  if (++g_run_telemetry_divider >= RUN_TELEMETRY_DIVIDER) {
    g_run_telemetry_divider = 0;
...
void setup() {
  ...
  g_timer->resume();
  Serial.println("READY,RIP_PPO_BONUS2_64_DIRECT_HW_GO_FIX,22");
}
