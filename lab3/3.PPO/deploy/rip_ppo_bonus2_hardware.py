#!/usr/bin/env python3
"""Hardware deployment panel for the direct 7->64->64->1 PPO actor.
...
A selected PPO Bonus-2 run directory is resolved to ``selected_best_model.zip`` first.
...
from rip_ppo_bonus2_sim_test import PPOBonus2Model, load_model, rip_points, set_line3d, set_point3d
...
def build_result_figure(result: HardwareResult, title_suffix: str = "Hardware direct PPO Bonus-2") -> Figure:
...
def default_output_paths(output_dir: str, duration: float) -> Tuple[str,str]:
    ...
    return str(directory/f"rip_ppo_bonus2_hardware_{tag}s_{stamp}.png"), str(directory/f"rip_ppo_bonus2_hardware_{tag}s_{stamp}.csv")
...
class ModelUploadThread(QtCore.QThread):
    ...
    def __init__(self, panel: "MainWindow", model: PPOBonus2Model, token: int, config: PanelConfig):
    ...
    @staticmethod
    def build_blob(model: PPOBonus2Model) -> bytes:
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("RIP Hardware PPO Bonus-2 Control Panel - Manual Refresh GO-Fix v22"); self.resize(1240,820)
        ...
        self.upload_in_progress=False; self.upload_thread: Optional[ModelUploadThread]=None; self.loaded_model: Optional[PPOBonus2Model]=None; self.last_model_source=""
        ...
        self.settings_path=Path.home()/".rip_ppo_bonus2_hardware_v22.json"
    ...
    def build_ui(self):
        ...
        model_group=QtWidgets.QGroupBox("PPO Bonus-2 model Selection & STM32 Deployment"); mg=QtWidgets.QGridLayout(model_group); self.model_path=QtWidgets.QLineEdit(); self.model_path.setPlaceholderText("Select a PPO Bonus-2 training run directory, selected_best_model.zip, .npz, or header")
        ...
        controller=QtWidgets.QGroupBox("direct PPO Bonus-2 Control & Velocity Estimation"); form=QtWidgets.QFormLayout(controller)
        ...
        note=QtWidgets.QLabel("The same PPO actor controls swing-up and balance at 200 Hz. Large-angle velocities use wrapped finite differences and blend into the Luenberger observer near upright. Firmware telemetry is 20 Hz and the panel redraws at 4 Hz. Model chunks use slow paced writes and retry on the current connection; port refresh/reconnect remain manual.")
        ...
        actions=QtWidgets.QGroupBox("Run"); av=QtWidgets.QVBoxLayout(actions); self.save_button=QtWidgets.QPushButton("SAVE / Upload PPO Bonus-2 model & Parameters"); self.go_button=QtWidgets.QPushButton("GO"); self.stop_button=QtWidgets.QPushButton("STOP")
        ...
        rr=QtWidgets.QHBoxLayout(); rr.addWidget(self.go_button); rr.addWidget(self.stop_button); av.addLayout(rr); self.status_label=QtWidgets.QLabel("Connect, select PPO Bonus-2 model, calibrate, then SAVE. Port refresh and reconnect are manual; protocol v22."); self.status_label.setWordWrap(True); self.state_label=QtWidgets.QLabel(); av.addWidget(self.status_label); av.addWidget(self.state_label); left.addWidget(actions)
    ...
    def mark_dirty(self,*_):
        self.settings_dirty=True; self.config_acknowledged=False; self.model_acknowledged=False; self.save_button.setText("SAVE / Upload PPO Bonus-2 model & Parameters *"); self.update_buttons()

    def choose_model_file(self):
        path,_=QtWidgets.QFileDialog.getOpenFileName(self,"Select PPO Bonus-2 model",str(Path(self.model_path.text() or Path.home()).expanduser()),"PPO Bonus-2 model (*.zip *.npz *.h)")
        if path:self.model_path.setText(path); self.preview_model()
    def choose_model_dir(self):
        path=QtWidgets.QFileDialog.getExistingDirectory(self,"Select PPO Bonus-2 training run directory",str(Path(self.model_path.text() or Path.home()).expanduser()))
        if path:self.model_path.setText(path); self.preview_model()
    def preview_model(self):
        try:
            m=load_model(self.model_path.text().strip())
            fmt = "canonical PPO Bonus-2 deploy NPZ" if m.source.suffix.lower() == ".npz" else ("SB3 ZIP (auto-adapted)" if m.source.suffix.lower() == ".zip" else "C header")
    ...
    def handle_line(self,line):
        ...
        if line.startswith("READY,"):
            parts=line.split(",")
            if len(parts)<3 or parts[2]!=str(PROTOCOL_VERSION):self.status_label.setText(f"Firmware mismatch. Flash the bundled rip_ppo_bonus2_hardware.ino protocol v{PROTOCOL_VERSION}."); return
        ...
        if line.startswith("ACK,CONFIG,"):
            ...
            self.settings_dirty=False; self.config_acknowledged=True; self.last_ack_config=config; self.save_button.setText("SAVE / Upload PPO Bonus-2 model & Parameters"); self.save_local_settings()
        ...
        if line.startswith("ARMED,"):
            ...
            if math.isfinite(initial_pwm):
                ...
                diagnostic=""
            self.status_label.setText("direct PPO Bonus-2 controller running: control 200 Hz, telemetry 20 Hz, panel redraw 4 Hz."+diagnostic); self.update_buttons(); return
    ...
    def save_and_send(self):
        ...
        fmt = "canonical PPO Bonus-2 deploy NPZ" if model.source.suffix.lower() == ".npz" else ("SB3 ZIP (auto-adapted)" if model.source.suffix.lower() == ".zip" else "C header")
    ...
    def start_model_upload(self,source,config,automatic=False,preloaded=None):
        ...
        self.status_label.setText("CRC-verified PPO Bonus-2 upload on the current serial connection..."); self.upload_thread=ModelUploadThread(self,model,upload_token,config); self.upload_thread.progress.connect(lambda value,text,token=upload_token:self.on_upload_progress(value,text,token))
    ...
    def show_result_dialog(self):
        ...
        dialog=QtWidgets.QDialog(self); dialog.setWindowTitle(f"RIP Hardware direct PPO Bonus-2 Result | {self.result.time[-1]:.3f} s Response"); dialog.resize(1080,800); layout=QtWidgets.QVBoxLayout(dialog); canvas=FigureCanvas(build_result_figure(self.result)); layout.addWidget(canvas); close=QtWidgets.QPushButton("Close"); close.clicked.connect(dialog.close); layout.addWidget(close); dialog.exec_()
