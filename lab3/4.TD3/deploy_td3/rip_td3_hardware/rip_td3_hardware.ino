/*
 * Rotary inverted pendulum direct TD3 controller.
 *
...
static const int MODE_TD3_DIFF = 1;
static const int MODE_TD3_BLEND = 2;
static const int MODE_TD3_OBSERVER = 3;
...
static_assert(sizeof(TD3FloatModel) == MODEL_BLOB_BYTES,
              "TD3FloatModel layout must match Python float_blob");

static ControllerConfig g_cfg = {
  0.0f, 150.0f, 20.0f, 35.0f, 0.18f, 0.25f,
  3110, 990, 0.00583730846f, 1
};
static TD3FloatModel g_model;
...
static bool td3Forward(const float rawObs[MODEL_INPUT], float* actionNorm, float* policyRaw) {
...
static void startControl(uint32_t runId) {
  ...
  // Capture one coherent initial measurement and make the first TD3 decision
  ...
  if (!td3Forward(initialObs, &initialAction, &initialRaw)) {
    motorBrake();
...
static void handleCommand(const String& raw) {
  ...
  if (line == "HELLO") { Serial.println("READY,RIP_TD3_64_DIRECT_HW_GO_FIX,22"); return; }
...
static void processControlTick() {
  ...
  if (!td3Forward(compactObs, &actionNorm, &policyRaw)) {
    stopControl();
  ...
  int mode = blend <= 0.01f ? MODE_TD3_DIFF :
             (blend >= 0.99f ? MODE_TD3_OBSERVER : MODE_TD3_BLEND);
  noInterrupts();
  g_step = step + 1;
  g_pwm = pwmApplied;
  g_mode = mode;
...
void setup() {
  ...
  g_timer->resume();
  Serial.println("READY,RIP_TD3_64_DIRECT_HW_GO_FIX,22");
}
