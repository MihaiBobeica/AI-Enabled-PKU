...
class CartPoleRIPSim2RealEnv(CartPoleRIPEenv):
    ...
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.nominal_init_cfg = copy.deepcopy(self.cfg.init_state)
        self.nominal_use_lpf_velocity = bool(getattr(self.cfg, 'use_lpf_velocity', False))
        self.nominal_velocity_lpf = float(getattr(self.cfg, 'velocity_lpf', 0.25))
    ...
    def get_episode_randomization(self) -> Dict[str, Any]:
        return self.episode_randomization.to_dict()
    ...
    def _sample_episode_actuator_and_sensor(self) -> None:
        ...
        if not getattr(dr, "enabled", False) or level <= 0.0:
            ...
            snap.use_lpf_velocity = self.nominal_use_lpf_velocity
            snap.velocity_lpf = self.nominal_velocity_lpf
        ...
        # Velocity-estimator randomization around the nominal DQN setting.
        # At level=0 the nominal setting is retained. At level=1, the configured
        # probability is the probability of using LPF.
        target_prob = float(np.clip(dr.use_lpf_velocity_probability, 0.0, 1.0))
        nominal_prob = 1.0 if self.nominal_use_lpf_velocity else 0.0
        prob = nominal_prob + level * (target_prob - nominal_prob)
        snap.use_lpf_velocity = bool(self.np_random.random() < prob)
        lo_full, hi_full = map(float, dr.velocity_lpf_range)
        lo = self.nominal_velocity_lpf + level * (lo_full - self.nominal_velocity_lpf)
        hi = self.nominal_velocity_lpf + level * (hi_full - self.nominal_velocity_lpf)
        if lo > hi:
            lo, hi = hi, lo
    ...
    def _randomize_initial_distribution(self) -> None:
        """Randomize the downward-start distribution around its nominal values.

        level=0 exactly reproduces the supplied DQN configuration.  Increasing
        levels expand the admissible standard-deviation interval toward the
        configured full range, without changing the downward mean angle.
        """
        dr = self.randomization
        base = self.nominal_init_cfg
        self.init_cfg = copy.deepcopy(base)
        self.cfg.init_state = self.init_cfg
        if not getattr(dr, "enabled", False) or self.randomization_level <= 0.0:
            return

        level = float(np.clip(self.randomization_level, 0.0, 1.0))

        def sample_around_nominal(nominal: float, full_range) -> float:
            full_lo, full_hi = map(float, full_range)
            lo = nominal + level * (full_lo - nominal)
            hi = nominal + level * (full_hi - nominal)
            if lo > hi:
                lo, hi = hi, lo
            return self._sample_uniform(lo, hi)

        self.init_cfg.theta_std_deg = sample_around_nominal(
            float(base.theta_std_deg), dr.init_theta_std_deg_range
        )
        self.init_cfg.alpha_std_deg = sample_around_nominal(
            float(base.alpha_std_deg), dr.init_alpha_std_deg_range
        )
        self.init_cfg.theta_dot_std = sample_around_nominal(
            float(base.theta_dot_std), dr.init_theta_dot_std_range
        )
        self.init_cfg.alpha_dot_std = sample_around_nominal(
            float(base.alpha_dot_std), dr.init_alpha_dot_std_range
        )
    ...
    def reset(self, *, seed: Optional[int] = None, options: Optional[dict] = None):
        ...
        # Preserve the nominal DQN LPF at level=0. At larger randomization
        # levels, occasionally toggle it and randomize its coefficient.
        self.use_lpf_velocity = bool(self.episode_randomization.use_lpf_velocity)
        self.cfg.use_lpf_velocity = self.use_lpf_velocity
        self.velocity_lpf = float(self.episode_randomization.velocity_lpf)
        self.cfg.velocity_lpf = self.velocity_lpf
