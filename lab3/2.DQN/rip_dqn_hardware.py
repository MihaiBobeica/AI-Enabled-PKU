#!/usr/bin/env python3
"""Hardware deployment panel for the MATLAB-aligned RIP DQN.

The visual layout, calibration, SAVE/GO workflow, 200 Hz telemetry, result plots
and CSV format follow the Class-9 LQR/MPC panels.  A student may select a .pt
checkpoint, model_weights.h, or a complete run directory.  SAVE uploads that
specific network into STM32 RAM and then sends the experiment configuration.

The firmware uses difference+LPF velocities while the pendulum is far from the
upright equilibrium (matching DQN training), then initializes and blends to the
fixed discrete Luenberger observer in the local upright region.
"""
...
from rip_dqn_sim_test import DQNModel, load_model, rip_points, set_line3d, set_point3d
...
ARM_Z = MOTOR_HEIGHT
PROTOCOL_VERSION = 8
MODE_NAMES = {0: "DISABLED", 1: "DQN_DIFF", 2: "ESTIMATOR_BLEND", 3: "DQN_LUENBERGER"}
...
def build_result_figure(result: HardwareResult, title_suffix: str = "Hardware DQN") -> Figure:
...
def save_result_csv(result: HardwareResult, path: str) -> None:
    ...
    with open(path, "w", newline="", encoding="utf-8") as handle:
        writer=csv.writer(handle); writer.writerow(["time_s","theta_rad","theta_dot_rad_s","alpha_rad","alpha_dot_rad_s","pwm","mode","blend"])
...
def default_output_paths(output_dir: str, duration: float) -> Tuple[str,str]:
    ...
    return str(directory/f"rip_dqn_hardware_{tag}s_{stamp}.png"), str(directory/f"rip_dqn_hardware_{tag}s_{stamp}.csv")
        ...
        while not self.stop_event.is_set():
            ...
            while b"\n" in buffer:
                ...
                if line.startswith("ACK,MODEL") or line.startswith("ERR,MODEL"):
                    self.model_reply_queue.put(line)
                else:
...
class ModelUploadThread(QtCore.QThread):
    ...
    Protocol v8 sends each model block in one self-contained ASCII line.  This
    ...
    @staticmethod
    def build_blob(model: DQNModel) -> bytes:
        parts = [
            np.ascontiguousarray(model.w1, dtype="<f4").tobytes(),
            np.ascontiguousarray(model.b1, dtype="<f4").tobytes(),
            np.ascontiguousarray(model.w2, dtype="<f4").tobytes(),
            np.ascontiguousarray(model.b2, dtype="<f4").tobytes(),
            np.ascontiguousarray(model.w3, dtype="<f4").tobytes(),
            np.ascontiguousarray(model.b3, dtype="<f4").tobytes(),
            np.ascontiguousarray(model.actions, dtype="<i2").tobytes(),
        ]
        blob = b"".join(parts)
        expected = (64 * 6 + 64 + 64 * 64 + 64 + 10 * 64 + 10) * 4 + 10 * 2
        if len(blob) != expected:
            raise AssertionError(f"Unexpected model blob size {len(blob)} != {expected}")
        return blob
    ...
    def wait_model_line(self, prefixes: tuple[str, ...], timeout: float) -> str:
        ...
        while time.monotonic() < deadline:
            try:
                line = self.panel.model_reply_queue.get(timeout=0.1)
            except queue.Empty:
                continue
    ...
    def run(self) -> None:
        try:
            ...
            begin = (
                ...
                f"{self.config.pwm_limit:.9g},{self.model.digest},{crc:08x}"
            )
            ...
            reply = self.command(end_line, done_prefix, retries=6, timeout=8.0)
            parts = reply.split(",")
            ...
            try:
                self.panel.send_line(
                    f"MODEL_HEX_ABORT,{PROTOCOL_VERSION},{self.token}"
                )
            except Exception:
                pass
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        super().__init__(); self.setWindowTitle("RIP Hardware DQN Control Panel - Student"); self.resize(1240,820)
        self.serial: Optional[serial.Serial]=None; self.serial_reader: Optional[SerialReadThread]=None; self.serial_lock=threading.Lock()
        ...
        self.upload_in_progress=False; self.upload_thread: Optional[ModelUploadThread]=None; self.loaded_model: Optional[DQNModel]=None; self.last_model_source=""
        ...
        self.latest={"time":0.0,"theta":0.0,"theta_dot":0.0,"alpha":math.pi,"alpha_dot":0.0,"pwm":0.0,"mode":0,"blend":0.0,"pot":0,"enc":0,"action":-1,"qmax":0.0}
        self.settings_path=Path.home()/".rip_dqn_hardware_student.json"
    ...
    def build_ui(self):
        ...
        model_group=QtWidgets.QGroupBox("DQN Model Selection & STM32 Deployment"); mg=QtWidgets.QGridLayout(model_group); self.model_path=QtWidgets.QLineEdit(); self.model_path.setPlaceholderText("Select .pt, model_weights.h, or a run folder")
        ...
        controller=QtWidgets.QGroupBox("Controller & Luenberger Observer"); form=QtWidgets.QFormLayout(controller)
        self.duration=self.dspin(30,0.1,600,3,1); self.pwm_limit=self.dspin(150,1,255,3,1); self.enter_deg=self.dspin(20,1,80,2,1); self.exit_deg=self.dspin(35,2,120,2,1)
        self.blend_alpha=self.dspin(0.18,0.001,1,4,0.01); self.velocity_lpf=self.dspin(0.25,0.001,1,4,0.01)
        form.addRow("Duration / s:",self.duration); form.addRow("PWM max:",self.pwm_limit); form.addRow("Observer enter / deg:",self.enter_deg); form.addRow("Observer exit / deg:",self.exit_deg)
        form.addRow("Observer blend λ:",self.blend_alpha); form.addRow("Large-angle velocity LPF β:",self.velocity_lpf)
        note=QtWidgets.QLabel("Far from upright: DQN uses wrapped angle differences + LPF, matching training. Near upright: the discrete Luenberger observer is initialized and blended into the same DQN input.")
        ...
        actions=QtWidgets.QGroupBox("Run"); av=QtWidgets.QVBoxLayout(actions); self.save_button=QtWidgets.QPushButton("SAVE / Upload Model & Parameters to STM32"); self.go_button=QtWidgets.QPushButton("GO"); self.stop_button=QtWidgets.QPushButton("STOP")
        ...
        rr=QtWidgets.QHBoxLayout(); rr.addWidget(self.go_button); rr.addWidget(self.stop_button); av.addLayout(rr); self.status_label=QtWidgets.QLabel("Connect, select model, calibrate, then SAVE."); self.status_label.setWordWrap(True); self.state_label=QtWidgets.QLabel(); av.addWidget(self.status_label); av.addWidget(self.state_label); left.addWidget(actions)
    ...
    def mark_dirty(self,*_):
        self.settings_dirty=True; self.config_acknowledged=False; self.model_acknowledged=False; self.save_button.setText("SAVE / Upload Model & Parameters to STM32 *"); self.update_buttons()
    ...
    def preview_model(self):
        try:
            m=load_model(self.model_path.text().strip()); self.model_info.setText(f"Resolved: {m.source}\n6→64→64→10 ReLU | actions {m.actions.tolist()} | LPF={m.velocity_lpf:g} | ID {m.digest}")
            self.velocity_lpf.setValue(m.velocity_lpf)
    ...
    def open_serial(self,port,automatic):
        ...
        try:
            sp=serial.Serial(port,BAUD,timeout=0.05,write_timeout=0.8); sp.reset_input_buffer(); sp.reset_output_buffer()
        except Exception as exc:
            if not automatic:QtWidgets.QMessageBox.critical(self,"Serial error",str(exc))
            return False
        self.serial=sp; self.serial_reader=SerialReadThread(sp,self.line_queue,self.error_queue,self.model_reply_queue); self.serial_reader.start(); self.connected=True; self.firmware_ready=False; self.active_port=port
        self.config_acknowledged=False; self.model_acknowledged=False; self.connection_label.setText(f"Opening: {port} @ {BAUD}"); self.connect_button.setText("Disconnect"); self.handshake_timer.start(350); QtCore.QTimer.singleShot(700,self.handshake_once); self.update_buttons(); return True
    def disconnect_serial(self,send_stop,update_status=True):
        ...
        if self.serial is not None and send_stop:
            try:
                with self.serial_lock:self.serial.write(b"STOP\n")
            except Exception:pass
        if self.serial_reader is not None:self.serial_reader.stop(); self.serial_reader.join(timeout=0.3)
        ...
        if update_status and not self.finishing:self.status_label.setText("Disconnected.")
        self.update_buttons()
    def send_line(self,line):
        if not self.connected or self.serial is None:return False
        try:
            with self.serial_lock:self.serial.write((line.strip()+"\n").encode("ascii")); self.serial.flush()
            return True
        except Exception as exc:
    ...
    def poll_queues(self):
        try:error=self.error_queue.get_nowait()
        except queue.Empty:error=None
        if error and self.connected:
            port=self.active_port; self.disconnect_serial(False); self.status_label.setText(f"Serial disconnected: {error}")
            if self.recording or self.run_pending:self.recording=False; self.run_pending=False; self.finalize_result()
            if port:self.schedule_auto_reconnect(port)
            return
        deadline=time.perf_counter()+0.006; count=0
        while count<2000 and time.perf_counter()<deadline:
            try:line=self.line_queue.get_nowait()
            except queue.Empty:break
            self.handle_line(line); count+=1

    def handle_line(self,line):
        if line.startswith("ACK,MODEL") or line.startswith("ERR,MODEL"):
            self.model_reply_queue.put(line); return
        if line.startswith("READY,"):
            parts=line.split(",")
            if len(parts)<3 or parts[2]!=str(PROTOCOL_VERSION):self.status_label.setText(f"Firmware mismatch. Flash the bundled rip_dqn_hardware.ino protocol v{PROTOCOL_VERSION}; protocol v8 ASCII-hex upload is required."); return
            self.firmware_ready=True; self.handshake_timer.stop(); self.connection_label.setText(f"Connected: {self.active_port} @ {BAUD}"); self.send_line("STATUS")
            if self.restore_after_ready and self.last_model_source and self.last_ack_config:self.start_model_upload(self.last_model_source,self.last_ack_config,automatic=True)
            else:self.status_label.setText("DQN firmware ready. Select model, calibrate and SAVE.")
        ...
        if line.startswith("ACK,CONFIG,"):
            ...
            self.settings_dirty=False; self.config_acknowledged=True; self.last_ack_config=cfg; self.save_button.setText("SAVE / Upload Model & Parameters to STM32"); self.save_local_settings()
            self.status_label.setText("Model and DQN/Luenberger parameters are active on STM32." if not automatic else "Serial reconnected; model and parameters restored automatically.")
        ...
        if line.startswith("ARMED,"):
            ...
            self.go_retry_timer.stop(); self.rows.clear(); self.last_recorded_step=-1; self.recording=True; self.run_pending=False; self.latest["time"]=0; self.status_label.setText("DQN firmware running at 200 Hz."); self.update_buttons(); return
        if line.startswith("DONE,") or line.startswith("STOPPED,"):
            try:run_id=int(line.split(",")[1])
            except Exception:return
            if run_id==self.current_run_id:self.complete_run("Configured DQN experiment duration completed." if line.startswith("DONE,") else "Run stopped.")
            return
    ...
    def handle_telemetry(self,line):
        ...
        if len(p)<12:return
        try:
            run_id=int(p[1]); step=int(p[2]); v={"time":step*CONTROL_DT,"theta":float(p[3]),"theta_dot":float(p[4]),"alpha":float(p[5]),"alpha_dot":float(p[6]),"pwm":float(p[7]),"mode":int(p[8]),"blend":float(p[9]),"pot":int(p[10]),"enc":int(p[11]),"action":int(p[12]) if len(p)>12 else -1,"qmax":float(p[13]) if len(p)>13 else 0.0}
        except ValueError:return
        ...
        if self.recording and run_id==self.current_run_id and step>self.last_recorded_step:
            self.rows.append([v["time"],v["theta"],v["theta_dot"],v["alpha"],v["alpha_dot"],v["pwm"],v["mode"],v["blend"]]); self.last_recorded_step=step

    def read_config(self):
        cfg=PanelConfig(float(self.duration.value()),float(self.pwm_limit.value()),float(self.enter_deg.value()),float(self.exit_deg.value()),float(self.blend_alpha.value()),float(self.velocity_lpf.value()),int(self.pot_up.value()),int(self.pot_down.value()),float(self.theta_scale.value()),int(self.motor_sign.currentData()))
        if cfg.observer_exit_deg<=cfg.observer_enter_deg:raise ValueError("Observer exit angle must exceed enter angle.")
    ...
    def config_line(self,cfg,token):
        return ",".join(["CONFIG",str(PROTOCOL_VERSION),str(token),f"{cfg.duration:.9g}",f"{cfg.pwm_limit:.9g}",f"{cfg.observer_enter_deg:.9g}",f"{cfg.observer_exit_deg:.9g}",f"{cfg.observer_blend_alpha:.9g}",f"{cfg.velocity_lpf:.9g}",str(cfg.pot_up),str(cfg.pot_down),f"{cfg.theta_rad_per_count:.12g}",str(cfg.motor_sign)])
    ...
    def save_and_send(self):
        ...
        try:
            ...
            if not source:raise ValueError("Select a .pt, model_weights.h, or run directory.")
            model=load_model(source)
        except Exception as exc:QtWidgets.QMessageBox.critical(self,"Model/settings error",str(exc)); return
        self.loaded_model=model; self.last_model_source=str(model.source); self.model_path.setText(str(model.source)); self.model_info.setText(f"6→64→64→10 | actions {model.actions.tolist()} | ID {model.digest}")
    ...
    def start_model_upload(self,source,cfg,automatic=False,preloaded=None):
        ...
        self.status_label.setText("Starting CRC-verified ASCII DQN upload to STM32 RAM..."); self.upload_thread=ModelUploadThread(self,model,self.model_token,cfg); self.upload_thread.progress.connect(self.on_upload_progress)
    ...
    def schedule_auto_reconnect(self,port):
        self.auto_reconnect_port=port; self.auto_reconnect_attempts=0; self.restore_after_ready=True; QtCore.QTimer.singleShot(600,self.attempt_auto_reconnect)
    def attempt_auto_reconnect(self):
        if self.connected or not self.auto_reconnect_port:return
        self.auto_reconnect_attempts+=1; self.refresh_ports(self.auto_reconnect_port)
        if self.open_serial(self.auto_reconnect_port,True):return
        if self.auto_reconnect_attempts<20:QtCore.QTimer.singleShot(min(3000,500+200*self.auto_reconnect_attempts),self.attempt_auto_reconnect)
        else:self.status_label.setText("Automatic reconnect failed.")
    ...
    def rows_to_result(self):
        ...
        d=np.asarray(self.rows,float); return HardwareResult(d[:,0],d[:,1],d[:,2],d[:,3],d[:,4],d[:,5],d[:,6].astype(int),d[:,7])
    ...
    def show_result_dialog(self):
        ...
        dialog=QtWidgets.QDialog(self); dialog.setWindowTitle(f"RIP Hardware DQN Result | {self.result.time[-1]:.3f} s Response"); dialog.resize(1080,800); layout=QtWidgets.QVBoxLayout(dialog); canvas=FigureCanvas(build_result_figure(self.result)); layout.addWidget(canvas); close=QtWidgets.QPushButton("Close"); close.clicked.connect(dialog.close); layout.addWidget(close); dialog.exec_()
    ...
    def load_settings(self):
        ...
        mapping={"duration":self.duration,"pwm_limit":self.pwm_limit,"observer_enter_deg":self.enter_deg,"observer_exit_deg":self.exit_deg,"observer_blend_alpha":self.blend_alpha,"velocity_lpf":self.velocity_lpf,"pot_up":self.pot_up,"pot_down":self.pot_down,"theta_rad_per_count":self.theta_scale}
