"""
Class 9 student hardware panel: swing-up plus constrained linear MPC stabilisation.

This is a minimal modification of the Class-8 panel.  Serial handling, sensor
calibration, 200 Hz telemetry, 3D view, SAVE/GO workflow and result logging are
retained.  The panel configures prediction horizon, diagonal Q, R, projected-gradient iterations and state estimation.
"""
...
class PanelConfig:
    ...
    estimator_mode: int = 0  # 0=differential+LPF, 1=Luenberger
...
def default_output_paths(output_dir: str, duration_s: float) -> Tuple[str, str]:
    ...
    return (
        os.path.join(output_dir, f"rip_mpc_hardware_{duration_tag}s_{stamp}.png"),
        os.path.join(output_dir, f"rip_mpc_hardware_{duration_tag}s_{stamp}.csv"),
    )
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("RIP Hardware MPC Control Panel - Student")
        ...
        self.settings_path = Path.home() / ".rip_mpc_hardware_student.json"
        self.build_ui()
        self.build_3d()
        self.load_settings()
    ...
    def build_ui(self) -> None:
        ...
        controller_form.addRow("MPC enter / deg:", self.enter_deg)
        controller_form.addRow("MPC exit / deg:", self.exit_deg)
        controller_form.addRow("Soft blend λ:", self.blend_alpha)
        left.addWidget(controller)

        mpc = QtWidgets.QGroupBox("Linear MPC Design & State Estimation")
        mpc_form = QtWidgets.QFormLayout(mpc)
        self.horizon = self.ispin(8, 4, 20)
        self.q_theta = self.dspin(1.0, 0.0, 100000.0, 6, 0.1)
        self.q_theta_dot = self.dspin(0.05, 0.0, 100000.0, 6, 0.01)
        self.q_alpha = self.dspin(80.0, 0.0, 100000.0, 6, 1.0)
        self.q_alpha_dot = self.dspin(2.0, 0.0, 100000.0, 6, 0.1)
        self.r_input = self.dspin(0.001, 0.000001, 1000.0, 6, 0.001)
        self.pgd_iterations = self.ispin(16, 1, 100)
        self.estimator_combo = QtWidgets.QComboBox()
        self.estimator_combo.addItem("Differential + low-pass filter", 0)
        self.estimator_combo.addItem("Luenberger observer", 1)
        self.velocity_lpf = self.dspin(0.25, 0.001, 1.0, 4, 0.01)
        mpc_form.addRow("Prediction horizon N:", self.horizon)
        mpc_form.addRow("Q theta:", self.q_theta)
        mpc_form.addRow("Q theta-dot:", self.q_theta_dot)
        mpc_form.addRow("Q alpha:", self.q_alpha)
        mpc_form.addRow("Q alpha-dot:", self.q_alpha_dot)
        mpc_form.addRow("Input weight R:", self.r_input)
        mpc_form.addRow("PGD iterations:", self.pgd_iterations)
        mpc_form.addRow("State estimator:", self.estimator_combo)
        mpc_form.addRow("Velocity LPF beta:", self.velocity_lpf)
        note = QtWidgets.QLabel(
            "SAVE makes the STM32 rebuild terminal P, condensed H/G and a safe PGD step. "
            "The LPF value is used in differential mode; Luenberger gains remain fixed."
        )
        note.setWordWrap(True)
        note.setStyleSheet("QLabel { color: #333; background: #f2f2f2; padding: 6px; }")
        mpc_form.addRow(note)
        left.addWidget(mpc)
    ...
    def connect_dirty_signals(self) -> None:
        ...
        self.estimator_combo.currentIndexChanged.connect(self.mark_dirty)
        self.estimator_combo.currentIndexChanged.connect(self.update_estimator_controls)
        self.update_estimator_controls()

    def update_estimator_controls(self, *_args) -> None:
        self.velocity_lpf.setEnabled(int(self.estimator_combo.currentData()) == 0)
    ...
    def handle_line(self, line: str) -> None:
        if line.startswith("READY,"):
            parts = line.split(",")
            if len(parts) < 3 or parts[2] != "4":
                self.status_label.setText("Firmware version mismatch. Flash the Class-9 MPC firmware (protocol v4).")
                return
            ...
            if self.restore_after_ready and self.last_ack_config is not None:
                ...
                self.status_label.setText("Class-9 MPC firmware ready. Calibrate, configure MPC and SAVE.")
            self.update_buttons()
            return
        if line.startswith("ACK,CONFIG,"):
            ...
            if automatic:
                ...
                self.status_label.setText("MPC and estimator parameters saved locally and active on STM32.")
            self.update_buttons()
            return
        ...
        if line.startswith("DONE,") or line.startswith("STOPPED,"):
            ...
            if run_id != self.current_run_id:
                return
            message = (
                "Configured MPC experiment duration completed."
        ...
        if line.startswith("ERR,"):
            if line in {"ERR,CONFIG_VALUE", "ERR,CONFIG_FORMAT", "ERR,MPC_BUILD"}:
                self.config_retry_timer.stop()
                self.pending_config = None
                self.pending_config_line = ""
    ...
    def read_config(self) -> PanelConfig:
        config = PanelConfig(
            ...
            q_theta=float(self.q_theta.value()),
            q_theta_dot=float(self.q_theta_dot.value()),
            q_alpha=float(self.q_alpha.value()),
            q_alpha_dot=float(self.q_alpha_dot.value()),
            r_input=float(self.r_input.value()),
            pgd_iterations=int(self.pgd_iterations.value()),
            estimator_mode=int(self.estimator_combo.currentData()),
        ...
        if not 4 <= config.horizon <= 20:
            raise ValueError("Prediction horizon N must be between 4 and 20.")
        weights = [config.q_theta, config.q_theta_dot, config.q_alpha, config.q_alpha_dot]
        if any((not math.isfinite(v)) or v < 0.0 for v in weights) or sum(weights) <= 0.0:
            raise ValueError("Q weights must be finite, non-negative, and not all zero.")
        if not math.isfinite(config.r_input) or config.r_input <= 0.0:
            raise ValueError("Input weight R must be greater than zero.")
        if config.estimator_mode not in (0, 1):
    ...
    def config_line(self, config: PanelConfig, token: int) -> str:
        values = [
            ...
            str(config.horizon),
            f"{config.q_theta:.9g}",
            f"{config.q_theta_dot:.9g}",
            f"{config.q_alpha:.9g}",
            f"{config.q_alpha_dot:.9g}",
            f"{config.r_input:.9g}",
            str(config.pgd_iterations),
            str(config.estimator_mode),
    ...
    def show_result_dialog(self) -> None:
        ...
        dialog.setWindowTitle(f"RIP Hardware MPC Result | {self.result.time[-1]:.3f} s")
    ...
    def load_settings(self) -> None:
        ...
        mapping = {
            ...
            "q_theta_dot": self.q_theta_dot,
            "q_alpha": self.q_alpha,
            "q_alpha_dot": self.q_alpha_dot,
        ...
        if "estimator_mode" in data:
            index = self.estimator_combo.findData(int(data["estimator_mode"]))
            if index >= 0:
                self.estimator_combo.setCurrentIndex(index)
